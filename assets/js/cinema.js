...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const cinemaGallery = document.getElementById("cinemaGallery");
    ...
    try {
        ...
        if (!cinemaGallery) {
            throw new Error(
                'Gallery container "#cinemaGallery" was not found.'
            );
        }
        ...
        const response = await fetch(
            "assets/image/cinema/cinema.json"
        );

        if (!response.ok) {

            throw new Error(
                `Unable to load cinema.json (${response.status})`
            );
        ...
        if (!Array.isArray(images)) {
            ...
            throw new Error(
                "cinema.json must contain an array."
            );
    ...
    (imageObj, index) => {
        return new Promise((resolve, reject) => {
            ...
            img.src = `assets/image/cinema/${imageObj.file}`;
        });
    }
);
        ...
        //     cinemaGallery.appendChild(link);
...
//     wrapper.className = "cinema-photo-item";

//     const link = document.createElement("a");
//     link.href = `assets/image/cinema/${imageObj.file}`;
//     link.className = "cinema-photo";
//     link.target = "_blank";

//     const img = document.createElement("img");
//     img.src = `assets/image/cinema/${imageObj.file}`;
//     img.alt = `cinema portrait ${index + 1}`;
//     img.loading = "eager";
...
//     cinemaGallery.appendChild(wrapper);
// });
...
cinemaGallery.addEventListener("click", function(e) {
    const photoItem = e.target.closest(".cinema-photo-item");
    if (photoItem) {
